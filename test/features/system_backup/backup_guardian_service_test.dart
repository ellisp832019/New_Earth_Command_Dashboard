...
void main() {
  test('backup guardian load snapshot reads config and status', () async {
    ...
    final backupFinishedAt = now.subtract(
    ...
    final moduleRoot = Directory(
      path.join(tempRoot.path, 'modules', 'system_backup'),
    );
    ...
    final backupTargetDir = Directory(
      path.join(backupDriveDir.path, 'NEW_EARTH_BACKUP'),
    );
    final reportsDir = Directory(path.join(backupTargetDir.path, 'reports'));
    final manifestsDir = Directory(
      path.join(backupTargetDir.path, 'manifests'),
    );
    ...
    final snapshot = await BackupGuardianService(
    ...
    expect(
      snapshot.latestReportPath,
      path.join(reportsDir.path, 'backup_guardian_20260607_060000.log'),
    );
    expect(
  ...
  test(
    'backup guardian load snapshot raises a freshness warning when the backup is stale',
    () async {
      final tempRoot = await Directory.systemTemp.createTemp(
      ...
      final runtimeDir = Directory(path.join(moduleRoot.path, 'runtime'));
      final sourceDir = Directory(path.join(tempRoot.path, 'source_drive'));
      final backupDriveDir = Directory(
        path.join(tempRoot.path, 'backup_drive'),
      );
      final backupTargetDir = Directory(
        path.join(backupDriveDir.path, 'NEW_EARTH_BACKUP'),
      );
      ...
      expect(snapshot.freshnessSummary, contains('freshness window'));
      expect(snapshot.notificationBanner, contains('Freshness check:'));
      expect(snapshot.healthState, BackupGuardianHealthState.amber);
    },
  );

  test(
    'backup guardian load snapshot warns when the latest manifest is missing',
    () async {
      final tempRoot = await Directory.systemTemp.createTemp(
  ...
  test('backup guardian verification readout explains manifest differences', () async {
    ...
    expect(
      snapshot.verificationDetails.any(
        (line) => line.contains('Target inventory fingerprint mismatch.'),
      ),
      isTrue,
    );
  ...
  test(
    'backup guardian load snapshot waits when the backup drive is missing',
    () async {
      final tempRoot = await Directory.systemTemp.createTemp(
      ...
      final sourceDir = Directory(path.join(tempRoot.path, 'source_drive'));
      final backupTargetDir = Directory(
        path.join(tempRoot.path, 'missing_drive', 'NEW_EARTH_BACKUP'),
      );
      ...
      expect(
        snapshot.warnings,
        contains('No backup history has been recorded yet.'),
      );
      expect(snapshot.errors, isEmpty);
    },
  );
