...
void main() {
  testWidgets('backup guardian dock host surfaces on wide layouts', (
    ...
    await tester.pumpWidget(
      _buildApp(location: RouteNames.dashboard, snapshot: _snapshot()),
    );
  ...
  testWidgets('backup guardian dock host hides on the full screen route', (
    tester,
  ) async {
    await tester.pumpWidget(
      _buildApp(location: RouteNames.backupGuardian, snapshot: _snapshot()),
    );

    await tester.pumpAndSettle();

    expect(find.text('Backup Guardian Dock'), findsNothing);
    expect(find.text('Open full view'), findsNothing);
  });
}

Widget _buildApp({
  required String location,
  required BackupGuardianSnapshot snapshot,
}) {
      ...
      GoRoute(
        ...
        builder: (context, state) => const Scaffold(
          body: MediaQuery(
            ...
            child: BackupGuardianDockHost(),
          ),
        ),
      ),
      GoRoute(
        path: RouteNames.backupGuardian,
        builder: (context, state) => const Scaffold(
          body: MediaQuery(
            data: MediaQueryData(size: Size(1440, 900)),
            child: BackupGuardianDockHost(),
...
BackupGuardianSnapshot _snapshot() {
