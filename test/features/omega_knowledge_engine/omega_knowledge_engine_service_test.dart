...
void main() {
  test(
    'omega knowledge engine falls back to safe defaults when config is missing',
    () async {
      final tempRoot = Directory.systemTemp.createTempSync(
        'omega-knowledge-engine-defaults-',
      );
      addTearDown(() {
      ...
      final service = OmegaKnowledgeEngineService(
  ...
  test('omega knowledge engine snapshot reads local sample outputs', () async {
    final tempRoot = Directory.systemTemp.createTempSync(
      'omega-knowledge-engine-snapshot-',
    );
    addTearDown(() {
      if (tempRoot.existsSync()) {
        tempRoot.deleteSync(recursive: true);
    ...
    await File(
      path.join(outputDir.path, 'repository_index.md'),
    ).writeAsString('# Repository Index\n\n- dashboard\n');
    await File(
      path.join(outputDir.path, 'repository_index.json'),
    ).writeAsString(
      '{"generated_at":"2026-06-24","repository_count":2,"files_scanned":11}',
    );
    await File(
      path.join(outputDir.path, 'code_learning_notes.md'),
    ).writeAsString('# Learning Notes\n');
    await File(
      path.join(outputDir.path, 'comment_suggestions.md'),
    ).writeAsString('# Comments\n');
    await File(
      path.join(outputDir.path, 'architecture_map.md'),
    ).writeAsString('# Architecture\n');
    await File(
      path.join(outputDir.path, 'project_memory.md'),
  ...
  test(
    'omega knowledge engine sample outputs support the preview and health sections',
    () async {
  ...
  test('omega knowledge engine saves and reloads local settings', () async {
    final tempRoot = Directory.systemTemp.createTempSync(
      'omega-knowledge-engine-settings-',
    );
    addTearDown(() {
      if (tempRoot.existsSync()) {
        tempRoot.deleteSync(recursive: true);
    ...
    expect(
      reloaded.obsidianExportDir,
      path.join(tempRoot.path, 'output', 'obsidian_export'),
    );
  ...
  test(
    'omega knowledge engine runScan fails safely when scanner script is missing',
    () async {
      final tempRoot = Directory.systemTemp.createTempSync(
        'omega-knowledge-engine-missing-script-',
      );
      addTearDown(() {
        if (tempRoot.existsSync()) {
          tempRoot.deleteSync(recursive: true);
  ...
  test(
    'omega knowledge engine snapshot stays readable when outputs are missing',
    () async {
      final tempRoot = Directory.systemTemp.createTempSync(
        'omega-knowledge-engine-empty-',
      );
      addTearDown(() {
        if (tempRoot.existsSync()) {
          tempRoot.deleteSync(recursive: true);
      ...
      expect(
        snapshot.commentSuggestionsText,
        contains('Sample output missing'),
      );
