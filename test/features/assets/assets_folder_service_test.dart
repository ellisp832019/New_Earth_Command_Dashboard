...
void main() {
  test('asset folder service bootstraps the journal folder and file', () async {
    final tempRoot = await Directory.systemTemp.createTemp('asset-folder-service-');
    addTearDown(() async {
    ...
    await File('${tempRoot.path}/config/local_paths.json').create(recursive: true);
    await File('${tempRoot.path}/config/local_paths.json').writeAsString(
      jsonEncode({
    ...
    expect(
      await File('${assetsRoot.path}/changes/asset_change_journal.csv').exists(),
      isTrue,
    );
  });
}
