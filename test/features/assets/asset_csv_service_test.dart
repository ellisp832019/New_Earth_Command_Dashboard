...
void main() {
  test('AssetCsvService preserves unknown columns when reading and writing',
      () async {
    ...
    final file = File(
      p.join(tempRoot.path, '01_EQUIPMENT_REGISTER', 'equipment_register.csv'),
    );
  ...
  test('AssetCsvService writes a backup before replacing an existing file',
      () async {
    final tempRoot = await Directory.systemTemp.createTemp(
    ...
    expect(await File('${file.path}.bak').readAsString(), contains('Legacy part'));
  });

  test('AssetRegisterRepository appends to equipment and parts registers',
      () async {
    final tempRoot = await Directory.systemTemp.createTemp(
    ...
    final assetsRoot = Directory(p.join(tempRoot.path, '18_ASSETS_EQUIPMENT_AND_PARTS'));
    ...
    final equipmentText = await File(
      p.join(assetsRoot.path, '01_EQUIPMENT_REGISTER', 'equipment_register.csv'),
    ).readAsString();
