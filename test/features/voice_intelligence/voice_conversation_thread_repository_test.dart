...
void main() {
  test('voice thread repository saves and loads a payload round trip', () async {
  ...
  test('voice thread controller restores the shared thread from SQLite', () async {
    final database = AppDatabase(NativeDatabase.memory());
    addTearDown(database.close);
    ...
    final controller = firstContainer.read(voiceConversationThreadProvider.notifier);
    ...
    await _waitForCondition(() async {
      final currentState = secondContainer.read(voiceConversationThreadProvider);
      return currentState.threadTitle == 'Dashboard Assistant' &&
          currentState.conversationEntries.isNotEmpty;
    });

    final hydratedState = secondContainer.read(voiceConversationThreadProvider);
