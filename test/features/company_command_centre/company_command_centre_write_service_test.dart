...
void main() {
  test('write service blocks writes while read-only is enabled', () async {
    ...
    expect(
      File(path.join(omegaRoot.path, 'notes', 'brief.md')).existsSync(),
      isFalse,
    );
  });
  ...
  test(
    'set read-only mode updates the config with a backup and audit entry',
    () async {
      final tempRoot = await Directory.systemTemp.createTemp(
      ...
      final updatedConfig =
          jsonDecode(await moduleConfig.readAsString()) as Map<String, dynamic>;
  ...
  test('export audit summary report writes a readable markdown file', () async {
    ...
    await auditFile.writeAsString(
      '${jsonEncode({'id': 'entry-1', 'timestamp': '2026-06-20T12:00:00Z', 'action': 'write_markdown', 'target_path': 'notes/brief.md', 'backup_path': 'backups/company_command_centre/20260620_120000/notes/brief.md', 'actor_label': 'Peter Ellis', 'note': 'Initial note', 'result': 'success'})}\n',
    );
