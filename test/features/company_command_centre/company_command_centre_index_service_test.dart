...
void main() {
  test(
    'scanner builds linked file indexes from markdown source files',
    () async {
...
''');
      ...
      final companyIndex =
          jsonDecode(await companyIndexFile.readAsString())
              as Map<String, dynamic>;
      expect(companyIndex['record_count'], 2);
      expect(companyIndex['source_exists'], isTrue);

      final actionIndex =
          jsonDecode(await actionItemsFile.readAsString())
              as Map<String, dynamic>;
      expect(actionIndex['record_count'], 2);

      final deadlineIndex =
          jsonDecode(await deadlinesFile.readAsString())
              as Map<String, dynamic>;
      expect(deadlineIndex['record_count'], 2);

      final grantIndex =
          jsonDecode(await grantsFile.readAsString()) as Map<String, dynamic>;
      ...
      final ipAssetsIndex =
          jsonDecode(await ipAssetsFile.readAsString()) as Map<String, dynamic>;
      expect(ipAssetsIndex['record_count'], 0);

      final evidenceIndex =
          jsonDecode(await evidenceFile.readAsString()) as Map<String, dynamic>;
