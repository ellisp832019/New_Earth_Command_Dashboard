...
final assetTreasuryLinkSummaryProvider =
  ...
  final maintenanceTable = await ref.watch(assetMaintenanceLogProvider.future);
  ...
  final receiptsMissingCount = [
    ...equipmentRows.where((row) {
      return _hasMoney(row['purchase_cost']) && !_hasValue(row['receipt_link']);
    }),
    ...ordersTable.rows.where((row) {
      return _hasMoney(row['total_cost']) && !_hasValue(row['receipt_link']);
    }),
  ].length;
  ...
  final linkedFinanceIdCount = [
    ...
    ...maintenanceTable.rows
        .where((row) => _hasValue(row['linked_finance_record'])),
  ].length;
  final repairReplacementValueTotal = brokenRows.fold<double>(
    0,
    (sum, row) => sum + (_parseMoney(row['replacement_value']) ?? _parseMoney(row['purchase_cost']) ?? 0),
  );
