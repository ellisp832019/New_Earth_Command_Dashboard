...
class MockVoiceAiProvider extends VoiceAiProvider {
  ...
  Future<VoiceTranscriptionResult> transcribe({
    ...
    return VoiceTranscriptionResult(
      ...
      mode: mode,
      providerLabel: config.canUseChat
          ? 'OpenAI-ready mock transcription'
          : 'Mock transcription',
      notes: config.canUseChat
  ...
  Future<MeetingSummaryResult> summarizeMeeting({
    ...
    if (normalizedTranscript.isEmpty) {
      return const MeetingSummaryResult(
        summary:
            'No transcript was supplied, so there is nothing to summarise yet.',
        decisions: <String>[],
        actions: <String>[],
        risks: <String>[],
        followUps: <String>[],
  ...
  Future<VoiceAssistantResponse> assist({
    ...
    if (intent == 'microgrow.status.read') {
      ...
      actions.add(
        const VoiceCommandAction(
          type: 'dashboard.assistant.reply',
          status: 'mock',
        ),
      );
      reply =
  ...
  Future<MicroGrowVoiceStatusResult> readMicroGrowStatus({
    ...
    if (normalizedQuery.contains('offline') ||
        normalizedQuery.contains('down')) {
      return const MicroGrowVoiceStatusResult(
        ...
        querySummary:
            'The node looks offline, so only a safe error can be shown.',
      );
    }

    final relays = <String, bool>{'ch1': false, 'ch2': false, 'ch3': true};

    final warnings = <String>[
      if (normalizedQuery.contains('temperature') ||
          normalizedQuery.contains('humidity'))
    ...
    return MicroGrowVoiceStatusResult(
      ...
      warnings: warnings,
      querySummary:
          'Mock read-only status returned for the requested MicroGrow query.',
    );
  }
...
class OpenAiVoiceAiProvider extends MockVoiceAiProvider {
  ...
  VoiceOpenAiTransport get _resolvedTransport {
    return _transport ??
        VoiceOpenAiTransport(
          apiKey: runtimeConfig.canUseOpenAi ? runtimeConfig.apiKey : null,
          baseUrl: runtimeConfig.chatBaseUrl,
        );
  }
  ...
  Future<VoiceTranscriptionResult> transcribe({
    ...
    if (!runtimeConfig.canUseChat) {
      return super.transcribe(mode: mode, prompt: prompt, config: config);
    }
    ...
    try {
      ...
      return VoiceTranscriptionResult(
        ...
        notes:
            '${_providerLabel()} was used. Recording and review stay local-first.',
      );
    } catch (_) {
      return super.transcribe(mode: mode, prompt: prompt, config: config);
    }
  ...
  Future<MeetingSummaryResult> summarizeMeeting({
    ...
    try {
      final summaryJson = await _resolvedTransport.completeJson(
        model: runtimeConfig.assistantModel,
        instructions:
      ...
      return MeetingSummaryResult(
        summary:
            ...
            '${_providerLabel()} meeting summary draft is ready for review.',
  ...
  String _providerLabel() {
    return runtimeConfig.providerMode == VoiceProviderMode.ollama
        ? 'Ollama local chat mode'
        : 'OpenAI chat mode';
  }
}
