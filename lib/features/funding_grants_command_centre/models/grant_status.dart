...
extension GrantStatusLabel on GrantStatus {
  ...
  static GrantStatus fromLabel(String value) {
    return GrantStatus.values.firstWhere(
      (status) => status.label.toLowerCase() == value.trim().toLowerCase(),
      orElse: () => GrantStatus.idea,
    );
  }

  bool get isTerminal =>
      this == GrantStatus.approved ||
      this == GrantStatus.rejected ||
      this == GrantStatus.paused ||
      this == GrantStatus.closed;
}
