# Next Actions

1. Review the completed round 2 workflow in `docs/next_20_task_workflow_round2.md`.
2. Use `docs/next_20_task_workflow_round3.md` for the post-polish feature sequence.
3. Keep the module on the current nested route structure unless a later task explicitly changes it.
4. Treat the local AI and RAG registries as the extension point for any future provider work.
5. Continue round 3 by polishing the dedicated report, profile, export, prompt, and settings subpages.
6. Use the export review controls as the handoff point for prompt-template picker work and later vault export refinements.
7. Keep comparison and change history focused on fast review rather than dense raw output.
8. Expand the graph drilldowns only if users still need more depth after the current filters land.
9. Use the architecture review card as the primary place for future graph-inspection tweaks.
